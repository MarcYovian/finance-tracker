...
const handleDelete = async (id: string) => {
  const confirmed = await confirm({
    title: 'Hapus Pola Berulang',
    message: 'Yakin ingin menghapus pola ini?',
    confirmText: 'Hapus',
    confirmColor: 'error'
  })
  if (!confirmed) return
      ...
      <UCard v-else-if="patterns.length === 0">
        <div class="text-center py-12">
          ...
          <p class="text-muted mb-4">
            Atur transaksi otomatis seperti gaji atau langganan
          </p>
          <UButton icon="i-lucide-plus" @click="openNewModal">
            Tambah Pola Pertama
          </UButton>
        ...
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <UFormField v-if="formData.transaction_type !== 'transfer'" label="Kategori">
            <USelectMenu
              ...
              :items="filteredCategories.map(c => ({ label: c.name, value: c.id }))"
              value-key="value"
          ...
          <UFormField :label="formData.transaction_type === 'income' ? 'Ke Sumber Dana' : 'Dari Sumber Dana'">
            <USelectMenu
              ...
              :items="fundSources.map(f => ({ label: f.name, value: f.id }))"
              value-key="value"
