...
export const useTransactions = () => {
  ...
  const createTransaction = async (transaction: TransactionCreate) => {
    ...
    try {
      // Prepare params - use null (not undefined) so all keys are included in RPC call
      const params = {
        p_type: transaction.type,
        p_amount: transaction.amount,
        p_description: transaction.description || null,
        p_category_id: transaction.category_id || null,
        p_source_fund_id: transaction.source_fund_id || null,
        p_destination_fund_id: transaction.destination_fund_id || null,
        p_transaction_date: transaction.transaction_date,
        p_notes: transaction.notes || null,
        p_recurring_pattern_id: transaction.recurring_pattern_id || null
      }

      // Use RPC function for atomic balance update
      // Type assertion needed because Supabase types don't allow nullable UUIDs
  ...
  const updateTransaction = async (
    ...
    try {
      ...
      // Prepare params - use null (not empty string) for proper SQL NULL handling
      const params = {
        p_transaction_id: id,
        p_amount: updates.amount,
        p_source_fund_id: updates.source_fund_id || null,
        p_destination_fund_id: updates.destination_fund_id || null,
